...
| Diffuse texture | ● | ● | ● | ● | Found next to the `.dts` by material name.  Export writes a `.png` beside the shape for every texture it names, so an exported shape carries its art — and **overwrites** what was there.  Untick **Export Textures** to write the `.dts` alone.  `mapping/texture_io.py:26` |
