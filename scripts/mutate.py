...
MUTATIONS = {
    ...
    "decal-fit": (
        ...
        # not the recall floor: a decal authored through create_decal fits back
        # to the property default, so the floor scores the same either way and
        # this went uncaught there.  The test below seeds a file the default
        # cannot reproduce, which is the only way to see the fit at all.
        ["test_the_fitted_coverage_rule_reaches_the_projector"],
    ),
    ...
    "trim-matters-to-tables": (
        ...
        "        pass",
        # not test_roundtrip: every fixture is written by this library now, and
        # its matters sets are built from the channels that exist, so there is
        # never a stray bit for the trim to remove.  The test below seeds one.
        ["test_pre_v22_clears_matters_bits_naming_things_the_shape_lacks"],
    ),
    ...
    # The two below exist because tests/fixtures/ is generated by this add-on
    # now (tests/fixtures/NOTES.md).  A reader test that reads a file the
    # writer produced can pass because both halves are wrong the same way, so
    # each side needs a mutation the *other* side's committed bytes catch.
    #
    # Read side: the fixtures are bytes on disk, written before the mutation
    # existed, so breaking only the reader has nothing to compensate it.
    "pre-v19-bounds-recompute": (
        "dtslib/old_reader.py",
        "            compute_mesh_bounds(mesh)",
        "            pass",
        ["test_v18_old_format_parses"],
    ),
    # Write side: the same bytes, from the other direction.  A writer that
    # stops stamping the version it was asked for cannot reproduce a fixture it
    # did not write, however consistent it is with the reader.
    "writer-version-stamp": (
        "dtslib/writer.py",
